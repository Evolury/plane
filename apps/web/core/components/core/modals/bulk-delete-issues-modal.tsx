...
export const BulkDeleteIssuesModal = observer(function BulkDeleteIssuesModal(props: Props) {
  ...
  const handleDelete: SubmitHandler<FormInput> = async (data) => {
    ...
    if (!data.delete_issue_ids || data.delete_issue_ids.length === 0) {
      setToast({
        ...
        title: t("toast.error"),
        message: t("toast.select_at_least_one_work_item"),
      });
      return;
    }
    ...
    await removeBulkIssues(workspaceSlug, projectId, data.delete_issue_ids)
      .then(() => {
        setToast({
          ...
          message: t("toast.work_items_deleted"),
        });
        handleClose();
      })
      .catch(() =>
        setToast({
          type: TOAST_TYPE.ERROR,
          title: t("toast.error"),
          message: t("toast.something_went_wrong"),
        })
      );
  };
  ...
  const issueList =
    issues.length > 0 ? (
      <li className="p-2">
        {query === "" && (
          <h2 className="mt-4 mb-2 px-3 text-11 font-semibold text-primary">{t("ui.select_work_items_to_delete")}</h2>
        )}
