    ...
    async (formData: Partial<TIssue>) => {
      ...
      await updateEntity(workspaceSlug.toString(), entityDetails.project_id, entityDetails.id, formData).catch(() => {
        setToast({
          ...
          title: t("toast.error"),
          message: `${isEpic ? "Epic" : t("common.work_item")} could not be updated. Please try again.`,
        });
      });
    },
      ...
      onSelect: (data) => {
        ...
        try {
          ...
          setToast({
            type: TOAST_TYPE.ERROR,
            title: t("toast.error"),
            message: `${entityDetails.is_epic ? "Epic" : t("common.work_item")} could not be updated. Please try again.`,
          });
        }
      },
