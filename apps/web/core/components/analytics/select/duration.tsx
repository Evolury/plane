  ...
  const options = ANALYTICS_DURATION_FILTER_OPTIONS.map((option) => ({
    ...
    content: (
      <div className="flex max-w-[300px] items-center gap-2">
        <span className="flex-grow truncate">{t(option.i18n_name)}</span>
      </div>
    ),
  }));
  return (
    <CustomSearchSelect
      ...
      label={
        <div className="flex items-center gap-2 p-1">
          ...
          {value
            ? t(ANALYTICS_DURATION_FILTER_OPTIONS.find((opt) => opt.value === value)?.i18n_name ?? "")
            : (placeholder ?? t("ui.duration"))}
        </div>
      }
    />
  );
